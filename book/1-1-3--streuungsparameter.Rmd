...
Meist reicht ein [Lageparameter](#lageparameter) als Zusammenfassung einer Datenreihe nicht aus, und man wünscht sich mehr Information. Streuungsparameter sind nun ein Maß dafür, wie sehr die Daten um einen Mittelwert schwanken. Auch hier gibt es verschiedene Möglichkeiten, solche Kennziffern zu berechnen. In der Praxis wird allerdings meist die Varianz bzw. ihre Wurzel, die Standardabweichung benutzt.

Da (zumindest die gebräuchlichen) Streuungsparameter in ihrer Definition immer irgendwo eine Differenz beinhalten, kann man sie nur für numerische Daten bestimmen, also diskrete oder stetige *Zahlen.* Bei einem [nominal- oder ordinalskalierten Merkmal](#merkmals-und-skalentypen) ist das nicht möglich.
...
```{r spannweite, fig.cap="Die Spannweite ist die Differenz zwischen dem größten und dem kleinsten Wert einer Datenreihe"}
knitr::include_graphics("img/spannweite.png")
```
...
```{r interquartilsabstand, fig.cap="Der Interquartilsabstand ist die Differenz zwischen dem 75%-Quantil und dem 25%-Quantil einer Datenreihe"}
knitr::include_graphics("img/interquartilsabstand.png")
```
...
(Die Klammern \(\lfloor\) und \(\rfloor\) bedeuten hier [Abrunden](#sec-abrunden))
